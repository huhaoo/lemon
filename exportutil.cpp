...
QString ExportUtil::getContestantHtmlCode(Contest *contest, Contestant *contestant)
{
    ...
    for (int i = 0; i < taskList.size(); i ++) {
        ...
        htmlCode += QString("%1 %2</span><br>\n").arg(tr("Task")).arg(taskList[i]->getProblemTile());
        ...
        if (taskList[i]->getTaskType() == Task::Traditional) {
            if (contestant->getCompileState(i) != CompileSuccessfully) {
                switch (contestant->getCompileState(i)) {
                    case NoValidSourceFile:
                        htmlCode += QString("&nbsp;&nbsp;%1</p>\n").arg(tr("Cannot find valid source file"));
                        break;
                    case CompileTimeLimitExceeded:
                        htmlCode += QString("&nbsp;&nbsp;%1%2<br>\n").arg(tr("Source file: "))
                                    .arg(contestant->getSourceFile(i));
                        htmlCode += QString("&nbsp;&nbsp;%1</p>\n").arg(tr("Compile time limit exceeded"));
                        break;
                    case InvalidCompiler:
                        htmlCode += QString("&nbsp;&nbsp;%1</p>\n").arg(tr("Cannot run given compiler"));
                        break;
                    case CompileError:
                        htmlCode += QString("&nbsp;&nbsp;%1%2<br>\n").arg(tr("Source file: "))
                                    .arg(contestant->getSourceFile(i));
                        htmlCode += QString("&nbsp;&nbsp;%1\n").arg(tr("Compile error"));
            ...
            htmlCode += QString("&nbsp;&nbsp;%1%2\n").arg(tr("Source file: ")).arg(contestant->getSourceFile(i));
        }
        
        htmlCode += "<table border=\"1\" cellpadding=\"1\"><tr>\n";
        htmlCode += QString("<th scope=\"col\" nowrap=\"nowrap\">%1</th>\n").arg(tr("Test Case"));
        htmlCode += QString("<th scope=\"col\" nowrap=\"nowrap\">%1</th>\n").arg(tr("Input File"));
        htmlCode += QString("<th scope=\"col\">%1</th>\n").arg(tr("Result"));
        htmlCode += QString("<th scope=\"col\" nowrap=\"nowrap\">%1</th>\n").arg(tr("Time Used"));
        htmlCode += QString("<th scope=\"col\" nowrap=\"nowrap\">%1</th>\n").arg(tr("Memory Used"));
        htmlCode += QString("<th scope=\"col\" nowrap=\"nowrap\">%1</th></tr>\n").arg(tr("Score"));
        ...
        for (int j = 0; j < inputFiles.size(); j ++) {
            for (int k = 0; k < inputFiles[j].size(); k ++) {
                ...
                htmlCode += QString("<td nowrap=\"nowrap\" align=\"center\">%1</td>\n").arg(inputFiles[j][k]);
                ...
                htmlCode += QString("<td align=\"center\">%1\n").arg(text);
    ...
    htmlCode += QString("<p><a href=\"#top\">%1</a></p>\n").arg(tr("Return to top"));
    return htmlCode;
}
...
void ExportUtil::exportHtml(QWidget *widget, Contest *contest, const QString &fileName)
{
    ...
    out << QString("<th scope=\"col\" nowrap=\"nowrap\">%1</th>\n").arg(tr("Rank"));
    out << QString("<th scope=\"col\" nowrap=\"nowrap\">%1</th>\n").arg(tr("Name"));
    for (int i = 0; i < taskList.size(); i ++)
        out << QString("<th scope=\"col\" nowrap=\"nowrap\">%1</th>\n").arg(taskList[i]->getProblemTile());
    out << QString("<th scope=\"col\" nowrap=\"nowrap\">%1</th></tr>\n").arg(tr("Total Score"));
